...
//        = TenantDropdown (react-select, creatable → skip 'Add "..."')
...
//   COLOUR (yarn) = CREATABLE dropdown → setCreatable (type + create the value)
//
...
test(`PART-1 BOM & WO — ${cfg.ipcs.length} IPC(s)`, async ({ page, playwright }) => {
  ...
  for (const job of cfg.ipcs) {
    ...
    // ── each component (created in Step0) ──
    for (const comp of job.components) {
      ...
      // material type (one-shot; the card gets created)
      ...
      // clear old material cards (renumber-safe: decrease the count)
      for (let g = 0; g < 20; g++) {
        const n = await allMats.count();
        if (n === 0) break;
        await materialCard(page, 1).getByText('Remove', { exact: true }).first().click();
        await expect(allMats, 'material count did not decrease').toHaveCount(n - 1, { timeout: cfg.timeout.element });
      }

      await expect(mtControl.or(specHeading).first(), 'neither dropdown nor card')
      ...
      if (await mtControl.isVisible().catch(() => false)) {
        ...
        await expect(menu, 'material-type menu did not open').toBeVisible();
        await menu.locator('[class*="-option"]').filter({ hasNotText: /^Add "/ })
          .filter({ hasText: new RegExp(`^\\s*${comp.materialType}\\s*$`, 'i') }).first().click();
        await expect(specHeading, 'material card was not created').toBeVisible({ timeout: cfg.timeout.element });
        console.log(`  material : ${comp.materialType} (card created)`);
      }

      // ── FABRIC ──
      if (comp.fabric) {
        const f = comp.fabric;
        await expect(allMats, 'there should be exactly one material card').toHaveCount(1);
        ...
        await page.waitForTimeout(600);                     // let the composition dropdown enable
        // COMPOSITION = manual TenantDropdown (enables after fiber+fabric). NOT auto.
        if (f.composition) { console.log('    → COMPOSITION...'); await setAny(page, scope, 'COMPOSITION', f.composition, 'COMPOSITION'); }

        // ── OPTIONAL FABRIC FIELDS (Phase-1: full coverage) — all manual dropdown/input ──
        // These 6 fields are hidden behind the "Advance Spec" button — click it first.
        // NOTE: there are several "Advance Spec" buttons on the page (work order QUILTING, foam...).
        // The fabric one is inside the "FABRIC SPECIFICATIONS" block — search within that block.
        const needAdvanced = f.fiberCategory || f.constructionType || f.weaveKnitType ||
                             f.machineType || f.origin || f.certificationRequirement;
        if (needAdvanced) {
          console.log('    → open Advance Spec (fabric)...');
          // closest common ancestor block of the FABRIC SPECIFICATIONS heading → the Advance Spec inside it
          ...
          if (!await advBtn.count()) {
            // fallback: if the block can't be located, the Advance Spec after the COMPOSITION field
          ...
          if (await advBtn.count()) {
            ...
            console.log('    ⚠ fabric Advance Spec button not found');
          }
        }
        // only fill fields when they're visible (advanced is open) — otherwise skip (so it doesn't crash)
        const tryAdv = async (label, val, tag) => {
          ...
          else console.log(`    ⚠ ${label} not visible — skipped (advanced section?)`);
        };
      ...
      if (comp.yarn) {
        ...
        await page.waitForTimeout(600);   // let the cascade dropdowns (composition/count/ply...) enable

        // Cascade-driven dropdowns (values depend on fiber+yarnType). COUNT SYSTEM = auto (skip).
        ...
        // COLOUR = CREATABLE dropdown ("Select or type Colour"). The value isn't in the list,
        // it's created via the 'Add "..."' row — so use setCreatable, not setAny.
        if (y.colour)          { console.log('    → COLOUR...');           await setCreatable(page, scope, 'COLOUR',      y.colour,          'YARN COLOUR'); }

        // Advanced group (SPINNING TYPE, FIBER CATEGORY, ORIGIN, CERTIFICATION) — behind "Advance Spec"
        // Yarn has its own "YARN SPECIFICATIONS" block — find the Advance Spec inside it.
        const needYarnAdv = y.spinningType || y.fiberCategory || y.origin || y.certificationRequirement;
        if (needYarnAdv) {
          console.log('    → open Advance Spec (yarn)...');
          ...
          if (await yAdvBtn.count()) {
            ...
            console.log('    ⚠ yarn Advance Spec button not found');
          }
        }
        // Advanced yarn fields: per source (TenantDropdown), SPINNING TYPE /
        // FIBER CATEGORY / ORIGIN are creatable dropdowns ("Select or type"),
        // but CERTIFICATION REQUIREMENT is a plain text input ("Enter certificate label").
        // So auto-detect: dropdown (-control present) → setCreatable, otherwise setAny.
        const tryYAdv = async (label, val, tag) => {
          if (!val) return;
          const box = field(scope, label);
          if (!await box.count()) { console.log(`    ⚠ ${label} not visible — skipped`); return; }
          console.log(`    → ${label}...`);
        ...
        // Extra generic fields (if y.fields is provided in the JSON)
      ...
      for (let w = 0; w < (comp.workOrders ?? []).length; w++) {
        ...
        await expect(woCard, `WO${w + 1} card not found`).toBeVisible();
      ...
      if (await noBtn.isVisible().catch(() => false)) {
        throw new Error('SAVE FAILED — "Not Saved"');
      }
    ...
    // back to selector (for the next IPC) + verify this IPC's BOM ✓
    await page.getByRole('button', { name: 'IPC Selector' }).first().click().catch(() => {});
    await waitForIpcSelector(page, cfg.timeout.page).catch(() => {});
    await expectBomDone(page, job.ipcIndex);          // did the ✓ appear on the portal card?
  }

  // ═══════════════════════════════════════════════════════════════
  //  QA BACKEND VERIFICATION — is every field of every component correct in the backend?
  //  (A UI "SAVED" isn't enough — did the data actually persist? THESE checks confirm it.)
  //  If a wrong value went into a dropdown but the form still saved, THIS will FAIL.
  // ═══════════════════════════════════════════════════════════════
  await test.step('QA VERIFY — BOM fields from backend (field-by-field)', async () => {
    const api = await apiContext(playwright);
    const ipoId = readRuntime(IPO_FILE)?.ipoId;
    if (!ipoId) {
      console.log('  ⚠ ipoId not found (.runtime/current-ipo.json) — backend verify skipped. Run the chain (01→02).');
      await api.dispose();
      return;
    }

    // read the saved BOM from the DRAFT sections endpoint
    const sections = await fetchSections(api, ipoId);

    // Safety: if sections are empty / shaped differently (backend migration/format),
    // don't hard-fail — give a clear diagnostic. The UI already confirmed SAVED
    // (validation passed); this layer is a stricter check BEYOND that.
    const bomwoRows = sections.filter(s => String(s.section || '').toLowerCase() === 'bomwo');
    if (bomwoRows.length === 0) {
      console.log(`  ⚠ 'bomwo' section not found in backend (sections: ${sections.length}).`);
      console.log(`     → BOM is already SAVED on the UI side; the draft-store format may differ.`);
      console.log(`     → Dumping one section to confirm the shape:`);
      console.log('     ' + JSON.stringify(sections.slice(0, 1)).slice(0, 300));
      await api.dispose();
      return;   // non-blocking: gave a diagnostic, test does not fail
    }
    ...
    await api.dispose();
    console.log('\n  ✅✅ ALL BOM fields BACKEND-VERIFIED — no wrong value persisted\n');
  });

  // ── progress report for ALL IPCs (portal gate track) ──
  const rep = await reportAllIpc(page);
  if (rep.bom < rep.total) {
    throw new Error(`Only ${rep.bom}/${rep.total} IPCs have BOM done — cannot proceed to Packaging`);
  }
  console.log(`✅ BOM & WO — ALL ${rep.total} IPCs complete. Next: Artwork/Cut&Sew, then Packaging.\n`);
});